...
The active diagnostic sketches are stored in this repository:
...
type-2 observations and type-3 IR/Hall interval reports. RX 1.1 adds type-4
acknowledgments for retained type-3 reports. These additions do not change the
raw packet's 250-byte layout.
...
The IR diagnostic family uses magic `0x4952`, version 1, and is multiplexed by
the `type` byte:

| Type | Size | Producer | Content |
|---:|---:|---|---|
| 1 | 250 | TX | Raw 1 kHz waveform batch; layout documented in §§1–4 |
...
unacknowledged report is evicted and `fevict` increments. Power loss or reset
clears retained reports. The store contains interval summaries, not the raw
waveform samples.
...
|---|---|---|
| `READY IR_SCOPE_ESPNOW_RX_1_1 channel=11 baud=921600 fusion_ack=1 mac=…` | RX boot | Banner for the ACK-capable receiver. |
| `FORMAT RX <millis> <rssi> <len> <crc16> <hex>` | RX boot (`RX:54`) | Self-describing header for the next kind. |
| `RX <millis> <rssi> <len> <crc16> <hex>` | per packet | The payload. `rssi` is dBm; `len` distinguishes 15-byte CTO echoes, 45-byte CTO status, and IR-family packets of 72, 86, or 250 bytes. |
| `STAT rx=… qdrop=… badlen=… ack=… ackerr=… ackqdrop=… q=… heap=…` | RX, every 5 s | Receiver and acknowledgment health. |
