...
|---|---|
| `firmware/programs/QUORUM/QUORUM.ino` | **THE control firmware — the only one.** QUORUM_1_6, bicameral: an automatic side and a manual side, three crossings between them (E-STOP, enlistment, release). Otto is still on 1.4. |
| `firmware/programs/MANUAL/` | REFERENCE ONLY, superseded. MANUAL_1_1 — QUORUM's navigator with the automatic chamber deleted rather than gated, so zero navigation-originated motor writes exist by construction. Useful to audit QUORUM against. Do not flash. |
| `firmware/programs/QUORUM/LocoConfig.h` | Profile selector — edit the include before flashing. Committed state selects **Otto (9950011)**. |
| `firmware/programs/QUORUM/LL_LocoConfig_995001{1,2}.h` | Otto's and Toby's profiles. |
| `firmware/programs/SENSORTEST/` | Marker measurement rig. Standalone, own WiFi/MQTT. |
| `firmware/programs/Spoke_IR_RSSI_survey/` | IR survey car **as flown**. Retained as the record; do not flash. |
| `firmware/programs/IR_TEST/` | IR survey car. 2026-08-05: first pass (filter + envelope, `IR_TEST_FILTER_AND_ENVELOPE.md`) field-tested on the 10-spoke wheel and superseded by the state/reacquisition pass (`IR_TEST_STATE_AND_REACQUISITION.md`) — interval median, honest UNAVAILABLE/REACQUIRING/VALID states, latch timeout, activity-gated decay, atomic NVS, raw-ring dump. Held for Sam's review, not flashed. |
