...
Operator direction, 2026-07-28:

> If the nav is not reliable, the system isn't viable. Fixing failure modes is
> a waste of time unless the fix addresses reliability.

Every proposed piece of work must answer one question:

**Does this make navigation more reliable, or does it manage the consequences
of unreliability?**

If the second, it is waste — because a system that handles being lost
gracefully is still not viable if it gets lost. Graceful degradation is not a
substitute for not degrading.

| | |
|---|---|
| **Reliability — do it** | M1's three changes. IR speed, twice over: it prevents the crawl that destroys the baseline, and it cross-validates position. Closed-loop stops, because per-consist hand-tuning is itself a reliability hazard. |
| **Consequence management — minimise to nothing** | Peer state machines. Degraded modes. Envelope arithmetic. Any behaviour designed for a lost locomotive that is still moving. |

One piece is irreducible: a lost locomotive must do *something*. Stop, publish,
hand to the operator. Three lines, not a subsystem.

**And this makes M1's crossing test the viability test for the project.** At
0.5% error and 15-marker reacquisition, LOST essentially never fires and the
bounds are a formality nobody reads. At 5% it fires most laps and no amount of
graceful handling rescues it. The question "how reliable is reliable enough?"
is answerable by measurement, and M1 is where it gets answered.

If M1 cannot be crossed, CTO is not viable on this sensor, and that is worth
knowing before M2 through M7 are built on top of it.
