...
// Motor-direction press awaiting this loco's confirmation echo. INFORMATIVE
// ONLY — no control's enabled state depends on it. The 1 s same-value window
// is a double-tap glitch guard, not a confirmation gate.
...
function sendInterval() {
  ...
  if (btn.disabled) return;                  // only CTO or reported MOTION disable it
  if (!pendingInterval) {
...
function sendDir(d) {
  if (isCto) return;
  var now = Date.now();
  // Double-tap glitch guard only: a same-value re-tap within 1 s is a bounce.
  // A different value, or a deliberate re-press later, always sends.
  if (pendingDir && pendingDir.value === String(d) && now - pendingDir.sentAt < 1000) return;
  pendingDir = { value: String(d), sentAt: now };
  dirNoConfirmUntil = 0;
  [0,1,2].forEach(function(i){
    var b = document.getElementById('dir-btn-'+i);
    if (b) b.classList.toggle('waiting', String(i) === String(d));
  });
  document.getElementById('dir-gate-note').textContent = 'WAITING FOR CONFIRMATION…';
...
function pollState(){
  fetch('/{{ slug }}/state').then(r=>r.json()).then(s=>{
    ...
    // ---- SET INTERVAL: the SOLE surviving MANUAL refusal besides E-stop
    // semantics — disabled only while the loco REPORTS motion (protects
    // declaration integrity, 2026-08-01 mid-motion declare corruption).
    // UNKNOWN motion and silence are warnings, not locks. ----
    var intervalGateNote = '';
    var intervalOk = true;
    if (isCto) { intervalGateNote = 'AUTO — DISPATCHER IN CONTROL'; intervalOk = false; }
    else if (motion === 'MOVING') { intervalGateNote = 'MOVING — STOP BEFORE DECLARING'; intervalOk = false; }
    ...
    // ---- Motor direction: confirmation echo, informative only. Resolves
    // from THIS loco's state/direction integer (SOLONAV 2_22 and QUORUM
    // alike); the DIRECTION nav event stamps freshness where present. ----
    if (pendingDir) {
    ...
    if (pendingDir)                            dirNote.textContent = 'WAITING FOR CONFIRMATION…';
    else if (Date.now() < dirNoConfirmUntil)   dirNote.textContent = 'NO CONFIRMATION — COMMAND MAY NOT HAVE ARRIVED';
