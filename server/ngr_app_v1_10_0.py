...
def _fresh_state():
    ...
    return {
        ...
        # from the 1 Hz alert / loopstat / nav events
...
def _apply_nav_state(lid, nav, mm=None):
    """Central nav bookkeeping: MM renders only while Otto reports TRACKING;
    nav falling to UNSET re-locks the interval/throttle gates."""
    st = loco_state[lid]
    prev = st["nav"]
    st["nav"] = nav
    _touch(lid, "nav")
        ...
        if sub == "online":
            ...
            try:
                ...
                if "auto" in d:
                    ...
                    _touch(lid, "auto")
                if "confidence" in d:
                    st["confidence"] = str(d["confidence"])
                ...
                if nav:
                    ...
                    # nav_ready exactly as the firmware derives it: a declared
                    # direction and a position it can name.
                    st["nav_ready"] = "1" if (sd in ("CW", "CCW") and nav == "TRACKING") else "0"
...
function updateDirButtons(d, live) {
  ...
  [0,1,2].forEach(function(i){
    ...
    if(live && String(i)===String(d)) btn.classList.add(classes[i]);
  });
}
...
function resetLocalSession(){
  // Otto rebooted: drop everything staged locally.
  pendingSend = null;
...
function pollState(){
  fetch('/{{ slug }}/state').then(r=>r.json()).then(s=>{
    ...
    // ---- Gate: throttle unlocks only on Otto's nav_ready confirmation ----
    var thrGateNote = '';
    var thrOk = false;
    ...
    updateDirButtons(s.direction, alive && ageOf(s,'direction') !== null);
    [0,1,2].forEach(function(i){
    ...
    // ---- Throttle display: Otto's confirmed throttle ----
    if (Date.now() > thrSliderLockedUntil) {
      var thrVal = (alive && ageOf(s,'throttle') !== null) ? s.throttle : '0';
      document.getElementById('thr-val').textContent = thrVal;
      document.getElementById('throttle-display').innerHTML = thrVal + '<span> / 255</span>';
      if (parseInt(thrVal) === 0) thrSlider.value = 0;
    }
    ...
    if (heardAge === null) {
      ...
      line = 'POSITION LOST — SUSTAINED CONTRADICTION'; cls = 'bad';
    } else if (pendingSend) {
      line = 'WAITING FOR OTTO TO CONFIRM INTERVAL…'; cls = 'warn';
    } else if (s.nav === 'TRACKING') {
