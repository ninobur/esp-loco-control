  ...
  <div class="panel-tight" id="panel-throttle">
    ...
    <div class="slider-row">
      <input type="range" min="0" max="255" value="0" step="1"
             ...
             oninput="sendCmd('throttle',this.value);
...
function pollState(){
  fetch('/{{ slug }}/state').then(r=>r.json()).then(s=>{
    ...
    // ---- Motor direction: PASSIVE DISPLAY ONLY (v1.10.3). The button is lit
    // from this loco's confirmed state/direction integer (SOLONAV 2_22 and
    // QUORUM alike). Nothing waits on it, nothing times out on it, and no
    // control's behaviour depends on it — an echo that never arrives costs
    // the operator a light, never a command. ----
