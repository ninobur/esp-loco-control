# ============================================================================
# NGR dashboard v1.10.10
#
# FIELD BUG, Otto, 2026-08-07: "the AUTO button does not work, and never has
# under QUORUM." Correct, and the fault was the console's, in three places.
# The operator pressed AUTO on the locomotive page and nothing happened. The
# locomotive was innocent: state/auto went to 1 and stayed there (verified on
# the broker, uptime 3393 s, no reboot). Three separate faults:
#
#   1. THERE IS NO GO BUTTON ON THE LOCOMOTIVE PAGE. cmd/auto only ENROLLS
#      the locomotive (QUORUM.ino:2535); motion requires a separate cmd/go.
#      The only GO buttons in the console live on the dispatcher page. So
#      pressing AUTO on /otto correctly enrolled Otto and then offered no way
#      to launch him. Fixed: GO and STOP now sit on the locomotive page,
#      inside the MODE panel, visible only once enrolled.
#
#   2. GO REFUSALS WERE INVISIBLE. QUORUM enforces seven gates on cmd/go and
#      publishes a reason for every refusal to station/* — ESTOP_ACTIVE,
#      WAIT_FOR_STOP, NEUTRAL_SELECT_DIRECTION, NOT_ENROLLED_IN_AUTO,
#      NO_QUORUM_DECLARE_POSITION, NO_POSITION_DECLARE_START_MM,
#      NO_SESSION_DIRECTION. This console never subscribed to station/# at
#      all, so a refused GO was indistinguishable from a dead button. The
#      firmware's own comment records that this silence was fixed once
#      before, in firmware; the console reintroduced it at the UI layer.
#      Fixed: station/# subscribed, and the latest event shown under MODE.
#
#   3. THE MODE DISPLAY LIED AFTER A DROPOUT. On a stale-then-recover flash
#      the page re-rendered mode from a local default instead of the
#      locomotive's reported state/auto, showing MANUAL while Otto was
#      enrolled. That errs in the dangerous direction: the console showed
#      LESS authority than the locomotive actually held. Fixed: the mode row
#      is driven by st.auto on every poll, and AUTO offers a RELEASE path.
#
# Also fixed, found while tracing the topics: the dispatcher page's "BOTH"
# GO/STOP published ngr/dispatcher/cmd/go with no locomotive suffix. QUORUM
# subscribes only to ngr/dispatcher/cmd/go/<id> (QUORUM.ino:1957), so the
# BOTH buttons were dead against QUORUM. They now fan out per locomotive.
#
# ---------------------------------------------------------------------------
...
def _fresh_state():
    ...
    return {
        ...
        # v1.10.10: the locomotive's own account of why it did or did not act
        # on a dispatcher command. QUORUM publishes every GO refusal here with
        # a reason; without this the operator sees a dead button.
        "station_event": "", "station_note": "", "station_ts": "",
    }
...
# v1.10.10 — the ONE narrow exception to the v1.10.0 drop-retained rule.
#
# Dropping retained deliveries killed the stale-tile ghosts and must stay. But
# it also throws away the locomotive's CURRENT authority state on every
# reconnect, and the default that stands in its place is a fabrication. That
# cost us the v1.10.9 Toby direction bug (default "1" = NEUTRAL shown as fact)
# and again the 2026-08-07 Otto report: state/auto was 1 on the broker while
# this console displayed MANUAL, because the retained 1 was dropped and the
# "0" default rendered. Showing LESS authority than the locomotive holds is
# the dangerous direction to be wrong in.
#
# So: for authority topics ONLY, accept the FIRST retained value seen after a
# connect, then resume dropping. Seeding is not a freshness event — the value
# is stored but never _touch()ed, so it displays as the locomotive's reported
# state without claiming it arrived live.
AUTHORITY_RETAINED = ("state/auto", "state/estop", "state/direction",
                      "state/session_direction", "state/start_interval")
loco_seeded = {lid: set() for lid in LOCO_IDS}
...
def on_mqtt_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        ...
        for lid in LOCO_IDS:
            loco_seeded[lid].clear()      # v1.10.10: re-seed authority once per connect
            base = f"ngr/loco/{lid}"
            ...
            # v1.10.10: state/station was ALREADY arriving under state/# — the
            # console simply threw it away. It is the locomotive's account of
            # every GO/STOP it accepted or refused, and why. Parsed below.
    else:
...
def on_mqtt_message(client, userdata, msg):
    ...
    with mqtt_lock:
        ...
        # render — EXCEPT the one-shot authority seed (v1.10.10, see
        # AUTHORITY_RETAINED). A dropped retained state/auto is why this
        # console showed MANUAL while Otto was enrolled.
        if retained:
            _log(lid, sub, payload, retained=True)
            if sub in AUTHORITY_RETAINED and sub not in loco_seeded[lid]:
                loco_seeded[lid].add(sub)
                key = SIMPLE_STATE.get(sub)
                if key:
                    st[key] = payload      # stored, deliberately NOT _touch()ed
            return
        ...
        if sub == "online":
            ...
            # v1.10.10: the locomotive explaining itself. QUORUM publishes on
            # TRANSITION only, so every one of these is a real event, and the
            # GO refusals (*_REFUSED with a note) are the ones the operator
            # was never shown. Kept verbatim — this is the locomotive's own
            # word, not the console's interpretation of it.
            try:
...
<div class="container">
  ...
  <div class="panel-tight">
    ...
    <div class="mode-row" id="mode-row">
      <a href="/{{ slug }}/mode/0" class="mode-btn man-active" id="mode-man">MANUAL</a>
      <a href="/{{ slug }}/mode/1" class="mode-btn" id="mode-auto">AUTO</a>
    </div>
    <div class="cto-note" id="cto-note" style="display:none;">AUTO — enrolled. Press GO to launch.</div>

    <!-- v1.10.10: GO/STOP live HERE, on the locomotive page. Before this the
         only GO buttons were on the dispatcher page, so pressing AUTO here
         enrolled the locomotive and then offered no way to start it — the
         "AUTO button does not work" report of 2026-08-07. Hidden in MANUAL
         because GO is refused when not enrolled; showing it would be
         offering a button that cannot work. -->
    <div id="auto-ops" style="display:none;margin-top:8px;">
      <div style="display:flex;gap:8px;">
        <button class="btn-go"   style="flex:1;" onclick="locoGo()">GO</button>
        <button class="btn-stop" style="flex:1;" onclick="locoStop()">STOP</button>
      </div>
    </div>

    <!-- The locomotive's own account of what it did with the last command.
         QUORUM publishes a reason for every GO refusal; this console never
         subscribed to it before 1.10.10. -->
    <div class="gate-note" id="station-note" style="display:none;"></div>
  </div>
...
// v1.10.10 — GO and STOP from the locomotive's own page.
//
// These publish ngr/dispatcher/cmd/go/<id>, which is the topic QUORUM
// actually subscribes to (QUORUM.ino:1957). The dispatcher page's per-loco
// buttons already used it; the locomotive page had no launch control at all,
// which is why pressing AUTO here appeared to do nothing.
//
// No client-side gating. QUORUM owns the seven GO gates and names its own
// refusals — a console that pre-judges them would be inventing a second
// eligibility gate, which is the thing the layer-boundary rule forbids.
function locoGo()   { fetch('/dispatcher/cmd/go/{{ lid }}',   {method:'POST'}).catch(e=>console.error(e)); }
function locoStop() { fetch('/dispatcher/cmd/stop/{{ lid }}', {method:'POST'}).catch(e=>console.error(e)); }
...
function pollState(){
  fetch('/{{ slug }}/state').then(r=>r.json()).then(s=>{
    ...
    // ---- Mode row ----
    // v1.10.10: MANUAL is now a live RELEASE control when enrolled, not dead
    // text. Previously the only way out of AUTO was the dispatcher page.
    var man = document.getElementById('mode-man');
    var auto = document.getElementById('mode-auto');
    if (isCto) {
      man.className = 'mode-btn'; auto.className = 'mode-btn auto-active';
      man.setAttribute('href', '/{{ slug }}/mode/0');
      auto.removeAttribute('href');
      document.getElementById('cto-note').style.display = '';
      document.getElementById('auto-ops').style.display = '';
    } else {
      man.className = 'mode-btn man-active'; auto.className = 'mode-btn';
      man.removeAttribute('href');
      auto.setAttribute('href', '/{{ slug }}/mode/1');
      document.getElementById('cto-note').style.display = 'none';
      document.getElementById('auto-ops').style.display = 'none';
    }

    // ---- The locomotive's account of the last command (v1.10.10) ----
    // QUORUM refuses GO on seven gates and names the reason every time. This
    // is that reason, verbatim, so a refused GO can never again be mistaken
    // for a dead button.
    var snote = document.getElementById('station-note');
    if (s.station_event) {
      var ev = s.station_event, note = s.station_note || '';
      var refused = ev.indexOf('REFUSED') >= 0;
      var human = {
        'ESTOP_ACTIVE'               : 'E-STOP IS ACTIVE — CLEAR IT FIRST',
        'WAIT_FOR_STOP'              : 'STILL ROLLING — STOP FIRST',
        'NEUTRAL_SELECT_DIRECTION'   : 'DIRECTION IS NEUTRAL — SELECT FORWARD OR REVERSE',
        'NOT_ENROLLED_IN_AUTO'       : 'NOT ENROLLED — PRESS AUTO FIRST',
        'NO_QUORUM_DECLARE_POSITION' : 'POSITION LOST — RE-DECLARE THE START INTERVAL',
        'NO_POSITION_DECLARE_START_MM': 'NO POSITION — DECLARE THE START INTERVAL',
        'NO_SESSION_DIRECTION'       : 'NO SESSION DIRECTION — SET CW OR CCW',
        'ALREADY_RUNNING'            : 'ALREADY RUNNING'
      }[note] || note;
      snote.textContent = refused
        ? (LOCO + ' REFUSED GO — ' + human)
        : (ev + (human ? ' — ' + human : '')) + '  ' + (s.station_ts || '');
      snote.style.color = refused ? '#f88' : '#8c8';
      snote.style.display = '';
    } else {
...
def dispatcher_cmd(subcmd):
    ...
    if subcmd not in allowed:
        return "", 204
    # v1.10.10: the bare "go"/"stop" (the console's BOTH column) published
    # ngr/dispatcher/cmd/go with no locomotive suffix. QUORUM subscribes ONLY
    # to ngr/dispatcher/cmd/go/<id> (QUORUM.ino:1957-1958), so those two
    # buttons were dead against QUORUM — silently. Fan them out per
    # locomotive so BOTH means both, and each locomotive still applies its
    # own seven gates and reports its own refusal.
...
def otto_mode(m):
    if m == 1:
        ...
        # v1.10.10: release back to MANUAL from the locomotive's own page.
        # cmd/auto 0 is door 3 of the four (QUORUM.ino:2537) — it drops
        # enrollment AND autoRunning and zeroes the auto throttle. Before
        # this the only release path was the dispatcher page.
        pub_loco("9950011", "auto", "0")
...
def toby_mode(m):
    if m == 1:
        ...
        pub_loco("9950012", "auto", "0")   # v1.10.10 release — see otto_mode
