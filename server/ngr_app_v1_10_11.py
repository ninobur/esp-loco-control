# ============================================================================
# NGR dashboard v1.10.11 — SESSION SURVIVES THE LINK
#
# Operator order 2026-08-08 ("Go"), from the field findings of the same
# day (FIELD_20260808_INAUGURAL_FINDINGS.md): connectivity loss must
# never disable operation the locomotive is ready to perform.
#
#   Finding A fix — a live bootid no longer resets the session. The
#     firmware republishes bootid on EVERY MQTT connect; 14+ reconnects
#     in one afternoon each wiped tallies and pre-flight while the
#     locomotive ran on undisturbed. Reboot detection is the alert's
#     uptime-regression test alone — a reconnect can never trip it.
#   Tallies self-heal — agree/disagree display from the locomotive's own
#     cumulative counts in the 1 Hz alert; console events only smooth
#     between beats.
#   Zombie last-wills neutralised — dispatcher online = heard-recently,
#     not the retained flag a stale LWT can flip.
#   Finding C fix — END CTO's suffix-less fossil stop now fans out.
#
# ---------------------------------------------------------------------------
# NGR dashboard v1.10.10 — THE AUTHORITY HANDOFF
#
# Implements docs/NGR_DASHBOARD_AUTHORITY_ALIGNMENT_SPEC.md Draft 5.1
# (five drafts, eight reviews; CODEX: approved for implementation).
# Companion firmware: QUORUM 1.10 (P11 enlistment guards, P13 response
# seq, P14 E-STOP direction preservation). This console may run against
# older firmware with tests T2/T6/T9 blocked, per spec §11.
#
#   P4  ENLISTED and RUNNING are two states. st["enlisted"] comes ONLY
#       from state/auto (autoEnrolled). st["auto"] (RUNNING) comes ONLY
#       from the 1 Hz alert (autoRunning). THE ALERT STREAM IS NEVER THE
#       SOURCE OF RECORD FOR ANY AUTHORITY STATE — that rule, not the
#       one-line fix, is what prevents Fault B recurring.
#   P5  All manual controls withdraw while enlisted — throttle,
#       DIRECTION, E-STOP, and the setup controls. Rendered from
#       reported state, never optimistically (R8: the absence of change
#       IS the refusal signal). The throttle slider zeroes on E-STOP
#       (R13) — the one sanctioned exception to "the slider never moves
#       by itself" (v1.10.2), because a stale slider value must not be
#       re-commanded when E-STOP clears.
#   P6  The console does not ISSUE cmd/auto 1 until ORIENTATION and
#       LOCATION are supplied. Sequencing, not authority: QUORUM 1.10
#       owns the refusals (P11) and this console displays them raw.
#   P8  Retained authority state is seeded PROVISIONALLY per connection
#       and promoted only when online=1 arrives; discarded on online=0.
#       Blank-until-proven: a powered-off locomotive must never display
#       ENLISTED (Draft-3 F3; field test T7).
#   P2/P3/P12  Dispatcher: BEGIN AUTO OPERATIONS / PAUSE AUTO OPERATIONS
#       / END AUTO OPERATIONS — RETURN TO MANUAL; BOTH fans out
#       per-locomotive (the suffix-less topic is an ESP-NOW-era fossil);
#       E-STOP ALL broadcasts, CLEAR is per-locomotive, and displayed
#       E-STOP state derives from retained state/estop only.
#   P7  state/station responses shown raw beside the dispatcher
#       controls, with the P13 seq when present.
#   P9  Enlistment lives on the loco page; release does NOT.
#   P10 Manual is never gated on telemetry. Unchanged and restated.
#
# ---------------------------------------------------------------------------
# NGR dashboard v1.10.9
#
# FIELD BUG, Toby, 2026-08-05: the dashboard said "FORWARD NOT ACCEPTED —
# TOBY REPORTS NEUTRAL (STOP FIRST: DIRECTION IS REFUSED WHILE MOVING)" while
# Toby was stationary at PWM 0, and FORWARD bounced back to NEUTRAL. Toby had
# in fact ACCEPTED the command — it moved afterwards. Both halves of that
# message were false, and the fault was mine, in three linked places:
#
#   1. _fresh_state() defaulted direction to "1" — which is NEUTRAL. A
#      fabricated value, indistinguishable from a reported one.
#   2. The firmware publishes state/direction RETAINED, and this dashboard
#      drops retained messages (the v1.10.0 rule that killed the stale-tile
#      ghosts). So after a console restart — I restarted it to deploy
#      v1.10.8 twenty minutes earlier — the locomotive's real direction was
#      dropped and the fabricated NEUTRAL stood in for it.
#   3. A DIRECTION nav event called _touch('direction'), stamping that
#      fabricated value as FRESH. The dashboard then reported, with
#      confidence, a direction the locomotive had never sent.
#
# This is precisely the failure v1.10.0 exists to prevent — a remembered
# number displayed as though the locomotive had said it — reintroduced by me
# through a default value and a misplaced freshness stamp.
#
# Fixed: direction defaults to "--" (unknown, never NEUTRAL); a DIRECTION
# event may refresh a value already held but can never manufacture one; the
# display treats "--" as not-reported and shows the operator's own last
# command instead, saying so; and the mismatch note no longer blames motion
# unless motion is actually reported.
#
# Known consequence, not a defect: after a console restart the direction
# buttons show your last command until the locomotive next reports — which
# is honest, and better than asserting NEUTRAL.
#
# v1.10.8
#
# CODEX review of v1.10.7: one interaction between the throttle coalescer and
# E-STOP. A drag value parked in thrPending was resent when the in-flight
# request completed — so after an E-STOP the dashboard could republish a
# nonzero throttle AFTER the intended zero, and paint that number back over a
# slider showing 0.
#
# The locomotive was never at risk: QUORUM ignores cmd/throttle while
# estopped, and clearing E-stop drops to NEUTRAL. The defect was that the
# console would display a command it had just cancelled — on the one page
# whose entire premise is that it does not lie about what the locomotive has
# been told.
#
# zeroThrottle() now clears the coalescer, and if a request is still
# outstanding it queues a TRAILING ZERO rather than merely dropping the
# pending value: the in-flight command may still reach the broker after ours,
# so the last throttle published must be 0 whichever way the ordering falls.
# E-STOP's own publish is untouched — still first, still direct, still
# ungated.
#
# v1.10.7
#
# CODEX review of v1.10.6 confirmed both fixes are real and rated the version
# deployable, with one correction: THE BANNER COULD NOT SEE THE FAILURE IT
# WAS BUILT FOR. pollState() still ended with `.catch(e=>{})`, and a caught
# promise rejection fires neither window.onerror nor unhandledrejection — so
# an exception inside the render chain, which is precisely the "state polling
# froze the page" case, was swallowed silently.
#
# pollState, pollLog and pollCalStatus now route failures through pollFail(),
# which banners a render exception and stays quiet for an ordinary dropped
# request. That distinction is the point: a banner that fires on every WiFi
# blip would stop being read, and then it would be worth nothing when it
# mattered.
#
# v1.10.6
#
...
loco_seed = {lid: {} for lid in LOCO_IDS}          # sub -> provisional value
loco_seed_online = {lid: None for lid in LOCO_IDS} # None until online seen
...
def on_mqtt_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        ...
        for lid in LOCO_IDS:
            loco_seed[lid].clear(); loco_seed_online[lid] = None   # P8: per-connection
            base = f"ngr/loco/{lid}"
...
<div class="container">
  ...
  <div class="panel">
    <h2>AUTO OPERATIONS</h2>
    <!-- v1.10.10 P3: BEGIN means "assess your situation and perform the
         action your starting position requires within your orders" — not
         "move forward". PAUSE keeps the locomotive enlisted; END (below)
         is the only control that returns authority to MANUAL. -->
    <div class="cmd-grid">
      <div>
        <div class="col-header">BOTH</div>
        <button class="btn-go"   onclick="dc('go')">BEGIN AUTO OPERATIONS</button>
        <button class="btn-stop" onclick="dc('stop')">PAUSE AUTO OPERATIONS</button>
      </div>
      <div>
        <div class="col-header">OTTO <span id="d-otto-authority" style="font-size:10px;color:#888;"></span></div>
        <button class="btn-go"   onclick="dc('go/9950011')">BEGIN</button>
        <button class="btn-stop" onclick="dc('stop/9950011')">PAUSE</button>
        <div class="gate-note" id="d-otto-station" style="font-size:11px;min-height:14px;"></div>
      </div>
      <div>
        <div class="col-header">TOBY <span id="d-toby-authority" style="font-size:10px;color:#888;"></span></div>
        <button class="btn-go"   onclick="dc('go/9950012')">BEGIN</button>
        <button class="btn-stop" onclick="dc('stop/9950012')">PAUSE</button>
        <div class="gate-note" id="d-toby-station" style="font-size:11px;min-height:14px;"></div>
      </div>
    </div>
  </div>
  ...
  <div class="panel">
    <div class="cmd-grid">
      ...
      <div style="grid-column:span 2;">
        <button class="btn-end" onclick="endCto()">END AUTO OPERATIONS &mdash; RETURN TO MANUAL</button>
      </div>
    </div>
  </div>

  <div class="panel">
    <!-- v1.10.10 P12: SET broadcasts — everyone stops is right for an
         emergency. CLEAR is per-locomotive (a broadcast clear would
         execute the clear path on locomotives that were never stopped,
         including a manually running one). Displayed state derives from
         retained state/estop ONLY — never from the last command sent,
         never from the button press. -->
    <button class="btn-estop" onclick="dc('estop')">&#9888; E-STOP ALL</button>
    <div style="display:flex;gap:10px;margin-top:10px;">
      <div style="flex:1;text-align:center;">
        <div style="font-size:10px;color:#666;">OTTO E-STOP</div>
        <div class="status-val" id="d-otto-estop">&mdash;</div>
        <button id="d-otto-esclear" onclick="dc('estopclear/9950011')" style="display:none;
          background:#333;border:1px solid #b32020;color:#f88;font-size:11px;
          padding:5px 14px;border-radius:6px;cursor:pointer;">CLEAR OTTO E-STOP</button>
      </div>
      <div style="flex:1;text-align:center;">
        <div style="font-size:10px;color:#666;">TOBY E-STOP</div>
        <div class="status-val" id="d-toby-estop">&mdash;</div>
        <button id="d-toby-esclear" onclick="dc('estopclear/9950012')" style="display:none;
          background:#333;border:1px solid #b32020;color:#f88;font-size:11px;
          padding:5px 14px;border-radius:6px;cursor:pointer;">CLEAR TOBY E-STOP</button>
      </div>
    </div>
  </div>
...
function pollStatus(){
  fetch('/dispatcher/state').then(r=>r.json()).then(s=>{
    // v1.10.10 P4: mode badge distinguishes ENLISTED from RUNNING.
    // enlisted comes from state/auto; running (s.*_auto) from the alert.
    function mode(enl, run, ce){
      if (run==='1') return ce==='1' ? 'CE' : 'RUN';
      if (enl==='1') return 'ENL';
      return enl==='--' ? '--' : 'MAN';
    }
    document.getElementById('otto-mode').textContent = mode(s.otto_enlisted, s.otto_auto, s.otto_ce);
    document.getElementById('toby-mode').textContent = mode(s.toby_enlisted, s.toby_auto, s.toby_ce);
    ...
    // P4 authority chips beside the BEGIN/PAUSE columns
    function chip(enl, run){
      if (run==='1') return '(RUNNING)';
      if (enl==='1') return '(ENLISTED)';
      return '';
    }
    document.getElementById('d-otto-authority').textContent = chip(s.otto_enlisted, s.otto_auto);
    document.getElementById('d-toby-authority').textContent = chip(s.toby_enlisted, s.toby_auto);
    // P12: E-STOP state from retained state/estop only; CLEAR appears
    // only for a locomotive actually reporting E-stopped.
    function esa(id, v){
      var el=document.getElementById('d-'+id+'-estop'), b=document.getElementById('d-'+id+'-esclear');
      el.textContent = v==='1' ? 'STOPPED' : (v==='0' ? 'clear' : '\u2014');
      el.style.color = v==='1' ? '#f55' : '#7a7';
      b.style.display = v==='1' ? '' : 'none';
    }
    esa('otto', s.otto_estop); esa('toby', s.toby_estop);
    // P7: latest command response / station event, raw, with P13 seq.
    function stn(id, r){
      var el=document.getElementById('d-'+id+'-station');
      if (r && r.event){
        var refused = r.event.indexOf('REFUSED')>=0 || r.event==='STOP_IGNORED';
        el.textContent = r.event + (r.note?(' \u2014 '+r.note):'') +
                         (r.seq!=null?(' #'+r.seq):'') + (r.ts?('  '+r.ts):'');
        el.style.color = refused ? '#f88' : '#8c8';
      } else { el.textContent=''; }
    }
    stn('otto', s.otto_station); stn('toby', s.toby_station);
  }).catch(e=>{});
}
var logPausedUntil = {};
...
// A dropped request is ORDINARY — a WiFi blip, the Pi restarting, the phone
// waking. An exception thrown inside our own render code is not: it is the
// fault that stops every later DOM update and makes the page look frozen.
// Only the second kind earns a banner, or the banner becomes noise and stops
// being read. (CODEX review of v1.10.6: pollState ended with .catch(e=>{}),
// which swallowed exactly the failure the banner exists to catch — a caught
// rejection fires neither window.onerror nor unhandledrejection.)
...
function pollCalStatus() {
  fetch('/{{ slug }}/cal/status')
    .then(r => r.json())
    .then(data => { calRunning = data.running; updateCalBtn(data); })
    .catch(function(e){ pollFail('pollCalStatus', e); });
}
...
function zeroThrottle() {
  ...
  // AN E-STOP SUPERSEDES ANYTHING THE COALESCER IS HOLDING (v1.10.8, CODEX
  // review of 1.10.7). Without this, a drag value parked in thrPending was
  // resent when the in-flight request completed — republishing a nonzero
  // throttle AFTER the intended zero, and painting that number back over a
  // slider sitting at 0. The locomotive was never at risk (QUORUM ignores
  // throttle while estopped, and clearing E-stop drops to NEUTRAL), but the
  // dashboard would have been showing a command it had just cancelled, on
  // the one page whose whole premise is that it does not lie.
  //
  // If a request is still outstanding, queue a TRAILING ZERO rather than
  // merely dropping the pending value: the in-flight command may still land
  // at the broker after ours, so the last throttle published must be 0
  // whatever the ordering turns out to be.
  thrPending = thrInFlight ? '0' : null;
  // The zero itself goes out directly and ungated, exactly as before —
  // nothing above is allowed to delay it.
...
function pollState(){
  fetch('/{{ slug }}/state').then(r=>r.json()).then(s=>{
    ...
    // v1.10.10 P4: greying and authority display key on ENLISTED
    // (state/auto = autoEnrolled). s.auto is RUNNING (autoRunning, from
    // the alert) and is shown as activity, never used for authority.
    isCto = (s.enlisted === '1');
    ...
    // A direction is REPORTED only when the locomotive has actually sent the
    // integer. "--" means we have never received one — which happens
    // routinely, because the firmware publishes state/direction RETAINED and
    // this dashboard drops retained messages, so a console restart loses it
    // until the locomotive next changes direction or reconnects. Treating
    // "never heard" as a report is what made Toby appear to sit in NEUTRAL.
    ...
    // Only speak when the locomotive has genuinely reported something OTHER
    // than what was commanded. And do not invent a cause: the firmware
    // refuses a direction change while moving OR under AUTO, so name motion
    // only when motion is actually reported.
    ...
    // ---- Mode row (v1.10.10 P5/P6/P9/R8) ----
    // Rendered from REPORTED state only. Successful enlistment = AUTO
    // lights + controls grey; a refusal = nothing changes (R8: the
    // absence of change is the signal). No release control here (P9) —
    // END lives on the dispatcher console.
    var man = document.getElementById('mode-man');
    var auto = document.getElementById('mode-auto');
    var note = document.getElementById('cto-note');
    ...
    if (isCto) {
      ...
      note.textContent = isRunning ? 'AUTO — RUNNING (dispatcher holds this locomotive)'
                                   : 'ENLISTED — dispatcher holds this locomotive. Release is on the dispatcher console.';
      note.style.display = '';
    } else {
      ...
      if (preflit) {
        ...
        auto.removeAttribute('href');   // P6: request withheld, not refused
        note.textContent = 'AUTO needs pre-flight: set ORIENTATION (CW/CCW), then the start interval.';
        note.style.display = '';
      }
    }
    ...
    if (esBtn) {
      ...
      esBtn.style.opacity = isCto ? 0.35 : 1;
      esBtn.title = isCto ? 'Enlisted — E-STOP from the dispatcher console' : '';
    }

    // ---- R13: the slider zeroes on E-STOP so a clear cannot re-command
    // a stale value. The one sanctioned exception to "the slider never
    // moves by itself" (v1.10.2), by operator ruling.
    if (s.estop === '1') {
      var sl = document.getElementById('throttle-slider');
...
def dispatcher_state():
    # v1.10.11: online = "heard recently", not the retained online flag. Each
    # MQTT reconnect strands a zombie last-will (the old session's online 0
    # firing AFTER the new session's online 1), so the flag flips while the
    # locomotive streams continuously. Heard-age cannot be fooled that way.
    def _heard(lid):
        t = loco_rx[lid].get("heard")
        return "1" if (t is not None and time.monotonic() - t <= 10) else "0"
    with mqtt_lock:
        return jsonify({
            "otto_auto":    loco_state["9950011"]["auto"],
            "toby_auto":    loco_state["9950012"]["auto"],
            "otto_online":  _heard("9950011"),
            "toby_online":  _heard("9950012"),
            ...
            # v1.10.10: authority (P4), E-STOP state (P12), responses (P7)
            "otto_enlisted": loco_state["9950011"]["enlisted"],
            "toby_enlisted": loco_state["9950012"]["enlisted"],
            "otto_estop":   loco_state["9950011"]["estop"],
            "toby_estop":   loco_state["9950012"]["estop"],
            "otto_station": {k: loco_state["9950011"]["station_"+k] for k in ("event","note","seq","ts")},
            "toby_station": {k: loco_state["9950012"]["station_"+k] for k in ("event","note","seq","ts")},
        })

@app.route("/dispatcher/cmd/<path:subcmd>", methods=["POST"])
def dispatcher_cmd(subcmd):
    allowed = {"go","stop","go/9950011","go/9950012","stop/9950011","stop/9950012",
               "ce","estop","estopclear/9950011","estopclear/9950012"}
    if subcmd not in allowed:
        return "", 204
    # v1.10.10 P2: bare go/stop (the BOTH column) fan out per locomotive.
    # QUORUM subscribes only to ngr/dispatcher/cmd/go/<id>; the suffix-less
    # topic is an ESP-NOW-era fossil — the Dispatcher ESP32 used to fan it
    # out over radio, and when locomotives went direct to MQTT the fan-out
    # left with it. Flask is the only place it can live now.
    if subcmd in ("go", "stop"):
        for lid in LOCO_IDS:
            pub_dispatcher(f"{subcmd}/{lid}")
    # P12: E-STOP clear is PER-LOCOMOTIVE, on the loco's own cmd topic
    # (already subscribed by firmware). SET stays broadcast ("estop").
    elif subcmd.startswith("estopclear/"):
        pub_loco(subcmd.split("/")[1], "estop", "0")
    else:
...
def dispatcher_endcto():
    ...
    # v1.10.11 (field finding C): the bare suffix-less stop was an
    # ESP-NOW-era fossil no locomotive subscribes to. Fan out like P2.
    for lid in LOCO_IDS:
...
def otto_mode(m):
    # v1.10.10 P6: the console does not ISSUE cmd/auto 1 until ORIENTATION
    # and LOCATION have been supplied. Sequencing, not authority — QUORUM
    # 1.10 independently refuses (P11); this just avoids sending a request
    # known to be premature. P9: there is deliberately NO release here —
    # only the dispatcher's END returns a locomotive to MANUAL.
    if m == 1:
        with mqtt_lock:
            sd = loco_state["9950011"]["session_dir"]
            si = loco_state["9950011"]["start_interval"]
        if sd in ("CW", "CCW") and si not in ("UNSET", "", "000-000"):
            pub_loco("9950011", "auto", "1")
        # else: withheld; the page's pre-flight note explains (R8: no
        # optimistic change — nothing was sent, nothing changes).
    return redirect("/otto")

@app.route("/otto/estop")
def otto_estop():
    # v1.10.10 R4/P5: the loco-page E-STOP is a MANUAL-chamber control.
    # While enlisted, the locomotive is not the loco operator's to command;
    # an enlisted locomotive is stopped from the dispatcher console. The
    # E-STOP firmware door itself is untouched (decision 0013 door 1).
    with mqtt_lock:
        enl = loco_state["9950011"]["enlisted"]
...
def toby_mode(m):
    # See otto_mode — P6 sequencing, P9 no release.
    if m == 1:
        with mqtt_lock:
            sd = loco_state["9950012"]["session_dir"]
            si = loco_state["9950012"]["start_interval"]
        if sd in ("CW", "CCW") and si not in ("UNSET", "", "000-000"):
...
def toby_estop():
    # See otto_estop — R4/P5: inert while enlisted.
    with mqtt_lock:
        enl = loco_state["9950012"]["enlisted"]
