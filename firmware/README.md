...
| `QUORUM/QUORUM.ino` — 1.12C, **optional trace overlay (`QUORUM_TRACE`)** | Production control, compile-time-gated diagnostic capability (not a separate sketch — decision 0018's one-control-sketch rule is unaffected: the flag is undefined/OFF by default and every `QT_*` call site compiles to nothing when it is) | Otto/Toby; passive, synchronized Hall-waveform + navigation-decision recorder tapping QUORUM's own existing detector and decision path (no second sampler, no new task competing with `hallTask`), now including an operator-anchor command (`cmd/trace_anchor`, also compile-time-gated) for ground-truth landmarks; built to establish Toby's reliable-control case ahead of comparing Otto's navigation failures | **Built — compiles clean (`--warnings all`) trace OFF and trace ON, both locomotive profiles, including the anchor mechanism; host-tested (`firmware/QUORUM/tests/`, `tools/qt_*`, 2,139 checks); a Toby trace-ON binary is preserved and SHA-256-hashed. Never flashed, never run.** Trace-OFF output is byte-identical to a build with no trace code at all — see README_TRACE.md's "Compile results" | `firmware/QUORUM/README_TRACE.md`; `firmware/QUORUM/QuorumTrace.h`; this commit |
...
| `test-programs/HALL_WAVEFORM_TEST/HALL_WAVEFORM_TEST.ino` (+ `tools/hwt_receiver.py`, `hwt_decode.py`, `hwt_plot.py`) | Diagnostic / instrument | Otto/Toby; continuous 1 kHz raw Hall waveform recorder built from `LL_PM_Loco_ModuleC_v0_4_1.ino` with all Module C navigation, block decoding, mode framework, lockout and ESP-NOW behaviour removed. Manual Blynk driving and operator-stepped fixed-PWM driving retained; recorder has no motor or navigation authority. One sensor (GPIO 33); second channel compiled out | **INVESTIGATORY / UNAPPROVED. Development — NOT built for ESP32 (no toolchain available where it was written) and never flashed. Host tests pass: capture engine, decoder, control-authority audit** | `test-programs/HALL_WAVEFORM_TEST/README.md`; `CAPTURE_FORMAT.md`; this commit |
