...
| `test-programs/IR_SCOPE/IR_SCOPE.ino` (+ `IR_SCOPE_Plotter.py`, `IR_SCOPE_Replay.py`) | Diagnostic | 1 kHz raw-waveform scope for the merged-pulse question; runs the IR_DIAG detector verbatim and streams every sample with thresholds and state | **Flashed to the data car 2026-08-09 (operator-directed). First field capture found a telemetry-transport defect (LWT flapping, gaps to 88 s, 5 recovery reboots) — an instrument fault, not an optical result. Transport fix on `agent/ir-scope-review` awaiting CODEX review + bench soak; optical capture postponed until it passes** | PR #3; `test-programs/IR_SCOPE/README.md`; decision 0019; `docs/IR_DEV_REC/2026-08-09_IR_SCOPE_BUILD.md` |
