...
| `test-programs/GRILLERS_WAYSIDE/GRILLERS_WAYSIDE.ino` — 0.2 | Diagnostic / instrument | Stationary ESP32 at Grillers, observation only: Hall GPIO 32 (CW) / 35 (CCW) read the identity magnet pair (NN Otto, SS Toby, NS Hans, SN Franz) and report which side saw the loco first; serial only, no commands, no NAV role | **Development; not built in the repo. 0.1 was bench exercised 2026-09-24 and showed split events and false FRANZ, which 0.2 addresses. 0.2 is not yet bench verified. That bench capture has not been committed** | Header in sketch (0.2 change list); 0.1 at the parent commit |
