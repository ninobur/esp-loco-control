...
| `QUORUM/QUORUM.ino` — 1.14A | Production control | 1.14 plus operator corrections after the first paired sessions: leader departure independent of the follower (deadlock removed), follower hold gap 12→9 markers, CCW station landings one marker earlier, and the CTO mode layer (BUBBLE/UNPAIRED, CE reserved) | **Built 2026-08-14, NOT flashed.** Both profiles compile, replay suite green. Mode layer and these corrections are unreviewed; two-locomotive behaviour still only ever exercised in the field, never in the harness | `docs/QUORUM_1_14_IMPLEMENTATION_REPORT.md`; decisions 0030–0034 |
