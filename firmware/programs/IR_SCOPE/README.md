...
```bash
arduino-cli compile --fqbn esp32:esp32:esp32 --warnings all firmware/programs/IR_SCOPE
```
...
```bash
python3 firmware/programs/IR_SCOPE/IR_SCOPE_Plotter.py --broker 192.168.68.142 --window 10
```
...
```bash
python3 firmware/programs/IR_SCOPE/IR_SCOPE_Replay.py ~/NGR/irscope_logs/irscope_20260809_1.csv
```
...
```bash
python3 firmware/programs/IR_SCOPE/IR_SCOPE_Replay.py capture.csv --plot 0.50 --start 12 --duration 4
```
