...
// 20Q3: IR reaches NAVI as a same-epoch odometry point (IR Epoch model),
// not the superseded TRACKING-at-both-endpoints MotionPoint interval.
static ngr_nav::IrOdometryEpoch owner;
static ngr_nav::IrOdometryPoint mp(uint64_t pulses,uint64_t us){return {&owner,1,1,us,pulses,0,9652};}
int main(){
 Navigator n;
 // Operator declaration is authoritative, but a Hall lobe 100 ms later is still too early.
 n.declare(55,1,1000);NavObservation early{};early.openedAtMs=1100;early.polarity=polarityAt(56);assert(n.judge(early)==Ruling::NonLandmark);assert(n.status().navMm==55);
 // First plausible Hall point establishes MM56 and the IR anchor.
 NavObservation a{};a.openedAtMs=1700;a.polarity=polarityAt(56);a.odometry=mp(100,1700000);assert(n.judge(a)==Ruling::Advanced);assert(n.status().navMm==56);
 // Spurious field/lobe: valid IR says only ~48 mm; cannot be MM57 even with right polarity.
 NavObservation b{};b.openedAtMs=2400;b.polarity=polarityAt(57);b.odometry=mp(105,2400000);assert(n.judge(b)==Ruling::NonLandmark);assert(n.status().navMm==56);
 // True MM57 near mapped distance, deliberately wrong polarity: continuity wins.
 uint64_t p1=(spanMm(56,1)+5)/10;NavObservation c{};c.openedAtMs=3200;c.polarity=!polarityAt(57);c.odometry=mp(100+p1,3200000);assert(n.judge(c)==Ruling::AdvancedWithDiscrepancy);assert(n.status().navMm==57);
 // Stopped/repeated field: no wheel movement, cannot advance.
 NavObservation d{};d.openedAtMs=4000;d.polarity=polarityAt(58);d.odometry=c.odometry;d.odometry.capturedUs=4000000;assert(n.judge(d)==Ruling::NonLandmark);assert(n.status().navMm==57);
 // Deliberately miss MM58; a Hall event near cumulative MM57->59 accepts 59 and records one missed.
 uint32_t two=spanMm(57,1)+spanMm(58,1);uint64_t p2=(two+5)/10;NavObservation e{};e.openedAtMs=5500;e.polarity=polarityAt(59);e.odometry=mp((100+p1)+p2,5500000);assert(n.judge(e)==Ruling::Advanced);assert(n.status().navMm==59);assert(n.status().missedSinceLast==1);assert(n.status().evidence==EvidenceClass::Confirmed);
 {SansMmAdvance s;assert(n.takeSansAdvance(s)&&s.mm==58&&s.referenceMm==57);assert(!n.takeSansAdvance(s));}
 // Missed-and-advance must not drop the final Hall polarity discrepancy.
 Navigator n2;n2.declare(55,1,1000);NavObservation f{};f.openedAtMs=1700;f.polarity=polarityAt(56);f.odometry=mp(200,1700000);assert(n2.judge(f)==Ruling::Advanced);uint32_t twob=spanMm(56,1)+spanMm(57,1);uint64_t pb=(twob+5)/10;NavObservation g{};g.openedAtMs=3200;g.polarity=!polarityAt(58);g.odometry=mp(200+pb,3200000);assert(n2.judge(g)==Ruling::AdvancedWithDiscrepancy);assert(n2.status().navMm==58);assert(n2.status().evidence==EvidenceClass::PolarityDiscrepancy);
 ...
 // ---- 0.5: IR window is +/-15% --------------------------------------------
 {Navigator w;w.declare(100,1,0);NavObservation a0{};a0.openedAtMs=1000;a0.polarity=polarityAt(101);a0.odometry=mp(1000,1000000);assert(w.judge(a0)==Ruling::Advanced);
  const double span=spanMm(101,1);
  auto at=[&](double frac,uint32_t ms){NavObservation o{};o.openedAtMs=ms;o.polarity=polarityAt(102);o.odometry=mp(1000+uint64_t(span*frac/9.652+0.5),uint64_t(ms)*1000);return o;};
  ...
  Navigator w4=w;assert(w4.judge(at(1.17,3000))==Ruling::NonLandmark);assert(w4.status().navMm==102);
 ...
 auto run=[](Navigator& n,uint8_t physicalFirst,int8_t dir,int count,int flipAt,uint32_t& ms){
   ...
   for(int i=0;i<count;++i){NavObservation o{};ms+=1000;o.openedAtMs=ms;o.polarity=polarityAt(phys);if(i==flipAt)o.polarity=!o.polarity;
     Ruling r=n.judge(o);assert(r!=Ruling::NonLandmark&&r!=Ruling::NoPosition);phys=nextMarker(phys,dir);}
   return phys;};
 ...
 {Navigator n;n.declare(55,1,1000);NavObservation f{};f.openedAtMs=1700;f.polarity=polarityAt(56);f.odometry=mp(200,1700000);n.judge(f);
  uint32_t twob=spanMm(56,1)+spanMm(57,1);NavObservation g{};g.openedAtMs=3200;g.polarity=polarityAt(58);g.odometry=mp(200+(twob+5)/10,3200000);
  assert(n.judge(g)==Ruling::Advanced);assert(n.status().sequenceLength==2);
  assert(n.recovery().count()==3 && !n.recovery().entry(1).observed);}
 std::cout<<"20Q3 navigator checks passed ("<<corrected<<" offset corrections, "<<held<<" single-misread holds)\n";
}
