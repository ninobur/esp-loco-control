...
//   * the guard is 645 ms from the detection instant
...
class ExcursionDetector {
 public:
  ...
  bool sample(uint32_t nowMs, int16_t raw, bool mayAdapt, bool stopped = false,
    ...
    // re-declares every time the 645 ms guard expires, for as long as it
  ...
  void serviceBaseline(uint32_t nowMs, int16_t raw, bool mayAdapt,
    ...
    switch (bstate_) {
      case BState::WaitGuard:
        ...
        if (refractoryUntil_ && (int32_t)(nowMs - refractoryUntil_) < 0) return;
