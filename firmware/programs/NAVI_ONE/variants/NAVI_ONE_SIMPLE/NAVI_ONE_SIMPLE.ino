/*
 * NAVI_ONE_SIMPLE_HALL — experimental magnet-only navigator.
 * One sustained Hall departure is one magnet event, even if the locomotive
 * stops on it. The map checks polarity and stops on disagreement.
...
#define SKETCH_NAME    "NAVI_ONE_SIMPLE_HALL_20260917"
#define BUILD_CLASS    "EXPERIMENTAL_FIELD_TEST"
#define BUILD_SUBTITLE "Magnet-only navigator with one baseline attempt per interval"
...
struct Judged {
  uint32_t epoch, openedAtMs, priorGapMs;
  int16_t raw, baseline;
  uint8_t polarity, pwm;
};
struct HallDecisionMsg { HallDecision d; uint32_t t; int16_t raw; };
...
static void publishNav(const char* event,const Judged* j,Ruling r){
  ...
  char b[400];
  if (j) {
    const uint8_t encountered = r==Ruling::Advanced ? s.navMm : s.target;
    const int n=snprintf(b,sizeof(b),
      "{\"event\":\"%s\",\"state\":\"%s\",\"nav\":\"%s\","
      "\"nav_state\":\"%s\",\"mm\":%u,\"tgt\":%u,"
      "\"landmark\":\"%s\",\"dir\":\"%s\",\"ruling\":\"%s\","
      "\"why\":\"%s\",\"obs\":\"%c\",\"expected\":\"%c\","
      "\"opened_ms\":%lu,\"gap_ms\":%lu,\"raw_open\":%d,"
      "\"base_open\":%d,\"pwm_open\":%u,\"adv\":%lu,\"ref\":%lu}",
      event,navigator.positionKnown()?"NORMAL":"UNSET",
      navigator.positionKnown()?"NORMAL":"UNSET",navStateName(s.state),
      s.navMm,s.target,landmarkAt(encountered),
      s.navDir>0?"CW":(s.navDir<0?"CCW":"UNSET"),rulingName(r),
      r==Ruling::WrongMagnet?"POLARITY_MISMATCH":
      (r==Ruling::NoPosition?"NO_POSITION":"MAGNET"),
      poleChar(j->polarity),poleChar(polarityAt(encountered)),
...
static void hallTask(void*){
  ...
  for (;;) {
    ...
    if (d.event == HallEvent::Open) {
      // Count on sustained entry. A locomotive parked on this magnet cannot
      // produce another event until it actually leaves and reaches another.
      Judged j{myEpoch,d.openedMs,d.priorGapMs,raw,before,d.polarity,pwm};
...
void setup(){
  ...
  Serial.printf("[BOOT] SIMPLE HALL experiment — not field accepted.\n");
...
void loop(){
  ...
  while (hallDecisionQ && xQueueReceive(hallDecisionQ,&hd,0)==pdTRUE) {
    ...
    snprintf(b,sizeof(b),
      ...
      event,why,(unsigned long)hd.t,(unsigned long)hd.d.openedMs,
  ...
  if (hallEventDrops && navigator.positionKnown()) {
