...
inline const char* issueName(MotionIssue r) {
  switch(r) {
    ...
    case MotionIssue::Optical:return "OPTICAL_UNAVAILABLE";
...
struct MotionPoint {
  ...
  IrHealth health=IrHealth::NoSource;
};
struct MotionInterval {
  uint64_t pulses=0;
  double nominalMm=0;
  MotionIssue issue=MotionIssue::NoSource;
  IrHealth startHealth=IrHealth::NoSource;
  IrHealth endHealth=IrHealth::NoSource;
...
inline MotionInterval between(const MotionPoint& a,const MotionPoint& b) {
  MotionInterval r;
  r.startHealth=a.health;r.endHealth=b.health;
  ...
  // Instrument health is deliberately separate from movement state.
  // TRACKING and SIGNAL_STALE are both healthy.  SIGNAL_STALE means no pulse
  // completed recently; at a normal stop that is meaningful zero movement.
  if(!irAvailable(a.health) || !irAvailable(b.health)){r.issue=MotionIssue::Optical;return r;}

  // These counters identify measurement integrity changes during the interval.
  // A changed counter means some portion of the distance interval is not known
  // to be clean.  Do not silently convert that into zero movement.
...
class MovementSource {
 public:
  ...
  IrHealth health(uint64_t now) const {
    if(!paired_ || !have_ || exhausted_)return IrHealth::NoSource;
    if(now<arrival_ || now-arrival_>1000000)return IrHealth::LinkStale;
    return opticalHealth(last_.opticalReason);
  }
  bool available(uint64_t now) const {return irAvailable(health(now));}
  ...
  RxResult receive(const uint8_t* mac,const uint8_t* bytes,size_t length,uint64_t now) {
    ...
    MotionPoint p{w,now,0,frame_,MotionIssue::None,opticalHealth(w.opticalReason)};
    history_[head_]=p;head_=(head_+1)%64;if(used_<64)++used_;
  ...
  MotionPoint at(uint64_t eventUs,uint64_t now) const {
    ...
    if(now<arrival_ || now-arrival_>1000000){out.issue=MotionIssue::Stale;out.health=IrHealth::LinkStale;return out;}
