...
echo ""
echo "== gate 14: the continuous Hall recorder — ring, batching, wire format =="
$CXX "$here/gate_recorder.cpp" -o "$tmp/recorder"; "$tmp/recorder" "$tmp/gate_recorder.xhr"

echo ""
echo "== gate 15: the decoder, against the firmware's own bytes and damaged streams =="
python3 "$repo/tools/tests/test_xhr_decoder.py" "$tmp/gate_recorder.xhr"
