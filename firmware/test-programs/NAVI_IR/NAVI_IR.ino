/*
 * NAVI_IR 0.4 - Toby, experimental evidence-fusion navigation.
 * Hall observations and cumulative wheel travel feed recoverable hypotheses.
 * IR is an independent map-reality gate: valid travel can reject Hall
 * events that occur too early, and confident IR can reseed NAV after loss. Every decision publishes its evidence and consequences.
 * Manual field evaluation first. AUTO requires an explicit reviewed build.
 * 0.4: NAVI is the sole decider. Hall, timing, map, history, PWM and IR are evidence sources; IR can gate physically-too-early Hall events and support relocalization.
...
#define SKETCH_NAME    "NAVI_IR_0_4"
#define BUILD_CLASS    "DEVELOPMENT_NOT_FOR_FIELD"
#define BUILD_SUBTITLE "Hall NAV + IR reality gate + map recovery"
...
#error "NAVI_IR requires a per-locomotive PWM ceiling"
#endif
...
static void publishNav(const char* event,const Judged* j,Ruling r){
  ...
  if (j) {
    const int n=snprintf(b,sizeof(b),
      ...
      "\"seq_matches\":%u,\"movement_predict_mm\":%u,"
      "\"movement_predict_ms\":%lu,\"movement_predict_valid\":%u}",
      ...
      (unsigned)s.unresolvedCount,(unsigned)s.sequenceMatches,
      (unsigned)s.movementPredictedMm,(unsigned long)s.movementPredictedAtMs,
      s.movementPredictionValid?1:0);
...
static void publishDecision(const Judged& j,const NavStatus& before,Ruling result,
  ...
  int n=snprintf(b,sizeof(b),
    ...
    "\"ruling\":\"%s\",\"state\":\"%s\",\"ambiguity_count\":%u,"
    ...
    rulingName(result),navStateName(navigator.status().state),navigator.status().unresolvedCount,
    ...
    navigator.unresolved()?"EVALUATE_ALL_EVIDENCE":navigator.positionKnown()?"TRACK":"POSITION_WITHDRAWN");
  ...
  for(uint8_t i=0;i<h.count();++i){
    ...
    snprintf(b,sizeof(b),
      ...
      "\"distance_consistent\":%u,\"ir_interval\":\"%s\",\"authority\":\"CONDITIONAL_ONE_FAULT_MODEL\"}",
...
static void compareMovement(const Judged& j,const ngr_nav::MotionPoint& point){
  ...
  for(uint8_t i=0;i<h.count();++i){
    ...
    for(unsigned step=0;step<=2;++step){
      ...
      const char* hardVerdict="CANNOT_ASSESS";
      if(step==1 && elapsed<500)hardVerdict="TIMING_TOO_EARLY";
      if(step==1 && interval.usable() && interval.nominalMm < double(distance)/2.0)
        hardVerdict="IR_TOO_EARLY";
      else if(step==1 && interval.usable())hardVerdict="IR_POSSIBLE";
      char b[650];
      snprintf(b,sizeof(b),
        ...
        (unsigned long)j.serial,i,x.mm,mm,step,distance,pulses,nominal,residual,ngr_nav::issueName(interval.issue),hardVerdict,
...
static void applyTravelDirection(){
  ...
  if (d != 0 && d != navigator.status().navDir) {
    navigator.setDirection(d);        // invalidates the unsigned movement frame
    carryResetRequest();
...
static void handleCommand(const CmdMsg& c){
  ...
  if (!strcmp(leaf,"estop")) {
    ...
    if (!want) {
      ...
      warn("AUTO disabled pending supervised NAVI_IR station acceptance");
    ...
    if (!NGR_ENABLE_EXPERIMENTAL_AUTO || !hallReady) {warn("GO disabled pending supervised NAVI_IR acceptance");return;}
...
void setup(){
  ...
  Serial.printf("[BOOT] NAVI_IR 0.4 DEVELOPMENT — not field accepted.\n");
  ...
  const int bootLen=snprintf(b,sizeof(b),
    ...
    "\"ir_authority\":\"PROPOSE_ALTERNATIVES_NOT_HARD_EXCLUDE\","
    "\"recovery_events\":10,\"recovery_requires_distance\":1,\"fault_budget\":1,\"traffic_coordination\":0}",
  ...
  Serial.println("[BOOT] Manual evaluation: pair IR, declare direction/interval, then use throttle. AUTO gated.");
}
...
static void stationService(uint32_t now){
  ...
  if(!navigator.positionKnown() && !navigator.unresolved()){
    withdraw("NAV lost: controlled stop, operator restart required");return;
  }
  // Never restore cruise on ambiguity. Continue only a common station action.
...
void loop(){
  ...
  while(judgedQ && xQueueReceive(judgedQ,&j,0)==pdTRUE){
    ...
    if(result==Ruling::UnresolvedLimitStop && autoRunning)
      withdraw("NAV recovery exhausted: controlled stop; declare position");
    if(result==Ruling::Advanced && before.navDir && lastAdvanceMs){
    ...
    if(result==Ruling::Reestablished || result==Ruling::MovementReseeded || result==Ruling::Ambiguous)estMmPerS=0;
    if(result==Ruling::Advanced || result==Ruling::Reestablished || result==Ruling::MovementReseeded)lastAdvanceMs=j.openedAtMs;
