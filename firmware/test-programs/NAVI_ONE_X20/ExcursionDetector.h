...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// X20 ADDS ONE THING: THE STATION DWELL.
//
// X19's first AUTO run stopped Otto at Arches with the sensor 96-106 counts
// off its own measured resting level -- parked in a magnet's fringe field --
// and counted three magnets in three seconds while the locomotive stood still.
// Position advanced three markers without moving and the next real magnet
// contradicted the map, correctly, seven seconds after departure.
//
// The mechanism was noteRest. A displaced level that holds for one window plus
// one refractory IS measured as rest; after that the signal's own wander in
// and out of it departs by 70 counts and is an event. Grillers, same firmware
// and the same 30 s dwell, stopped in clean magnetic space and produced
// nothing. The difference was never the code -- it was where the sensor came
// to rest.
//
// SO: ONE PROPULSION FACT NOW REACHES THE DETECTOR, AND ONLY ONE.
//
//     `stopped` -- the station machine is holding AND the ramped motor PWM is
//     exactly 0. Nothing else. It is not a speed, not a threshold, not a
//     motion estimate, and it is false for every part of a station approach
//     including the whole zero ramp.
//
// STATION DECELERATION REMAINS NORMAL HALL OPERATION. There is a ramp after
// the stop command -- about twelve seconds at 200 ms a count -- and the
// locomotive is still traversing the station magnet through it. Detection is
// untouched for all of it. The dwell begins at PWM 0 and not one sample
// earlier.
//
// WHAT `stopped` DOES, IN FOUR PARTS
//
//   1. REST IS FROZEN. The anchor stays at the level measured WHILE MOVING.
//      This is the fix for Arches: the detector can no longer re-base its zero
//      onto the field it is parked in.
//
//   2. NO NEW CANDIDATE MAY BE DECLARED. Sampling, the ring, the histogram and
//      every diagnostic keep running, and anything that WOULD have been a
//      candidate is counted and published -- so the suppression can never be
//      silent. Nothing during a dwell advances navigation.
//
//   3. AN IN-FLIGHT WINDOW IS TRUNCATED, NOT EXTENDED. If a magnet was
//      encountered while moving and PWM reaches 0 before the 400 ms window
//      expires, measurement ENDS THERE and the candidate is emitted with what
//      it has. Stationary samples are NOT appended to moving ones: the
//      locomotive has stopped traversing the field, and waiting another 200 ms
//      does not create another 200 ms of spatial magnet information. Peak,
//      polarity and both widths are therefore taken from the moving portion
//      alone.
//
//      POLARITY SURVIVES THIS, and this is the easier case rather than the
//      harder one. The way a pole inverts -- see localRef() below -- is a
//      wrong zero making the detector fire on an arc's TRAILING edge. A
//      locomotive decelerating into a magnet has no trailing edge: the signal
//      rises toward the apex and then holds. Every moving sample sits on one
//      side of the local zero. In the limit, PWM reaches 0 one sample after
//      detection and the sum is the detection sample by itself -- which
//      already carries at least departCounts of SIGNED departure, because that
//      is what declared it. There is no case with no moving evidence.
//
//   4. THE REFRACTORY IS NOT CARRIED ACROSS THE STOP. A dwell breaks timing
//      continuity. Magnets are about a metre apart and the next real field can
//      be under the sensor within 500 ms of departure, depending on where the
//      locomotive stopped and how fast the ramp brings it up, so carrying a
//      500 ms detect-to-detect guard over a 30 s stop could blind it to a
//      legitimate magnet. The ordinary moving refractory is untouched for
//      ordinary moving passages.
//
// LEAVING THE DWELL: TWO CONDITIONS, DECIDED FROM THE APPROACH
//
// At PWM 0 the detector classifies where it came to rest against the rest it
// measured while moving:
//
//   A  REST           |raw - rest| < departCounts. Normal detection is armed
//                     the instant the station machine departs. No blind time.
//
//   B  IN_OLD_FIELD   the sensor is still displaced inside the field of the
//                     magnet that was ALREADY encountered while moving. The
//                     changing Hall values as the locomotive leaves that field
//                     are that same field, not a new magnet, so declaration
//                     stays refused and rest stays frozen until the signal is
//                     back within departCounts of it for persistSamples. Then
//                     OLD_FIELD_CLEAR, and normal detection arms immediately.
//
// The displaced value is used to classify the STATE. It is never used to infer
// a magnet -- that is exactly the inference Arches got wrong.
//
// THE HOLD IN B IS NOT BOUNDED BY A TIMER, deliberately: any cap would be the
// arbitrary post-dwell blind time this design exists to avoid, and a departure
// ramp can legitimately spend many seconds leaving a field. The cost is that a
// badly wrong frozen rest would hold detection off indefinitely. That is made
// LOUD rather than bounded -- diag/dwell carries the hold and its suppressed
// count, and state/loopstat carries hold_ms while it runs.
//
...
struct Excursion {
  ...
  // X20. The measurement ended because the ramped PWM reached 0, not because
  // the 400 ms window expired: peak, polarity and both widths are taken from
  // the MOVING portion alone and no stationary sample was appended to them.
  bool     stoppedShort   = false;
  // Samples measured AFTER the detection sample. 400 on an ordinary passage;
  // whatever the locomotive had left when it stopped, otherwise.
...
class ExcursionDetector {
 public:
  ...
  // "may declare a new magnet now". False through a dwell and through an
  // old-field hold, true everywhere else.
  ...
  void reset() {
    ...
    // stationary locomotive being told where it is -- whatever it was parked
    // in is no longer evidence about anything.
    dwell_ = false; oldFieldHold_ = false; restFrozen_ = false;
  ...
  // `stopped` is THE ONE PROPULSION FACT IN THE EVENT PATH (X20): the station
  // machine is holding and the ramped motor PWM is exactly 0. It defaults to
  // false, so every host gate and the 2026-09-15 replay -- neither of which
  // has a throttle in it -- exercise the identical X19 path.
  bool sample(uint32_t nowMs, int16_t raw, bool mayAdapt, bool stopped = false) {
    ...
    // The dwell edge is taken BEFORE noteRest, so the first stationary sample
    // can never reach the rest estimator.
    ...
    if (acquiring_) {
      ...
      // THE MEASUREMENT ENDS AT PWM 0. It is not extended with stationary
      // samples: the locomotive has stopped traversing the field, so there is
      // no more spatial information to wait for.
    ...
    // X20. A STATION DWELL, AND THE OLD FIELD IT MAY HAVE STOPPED IN, DECLARE
    // NOTHING. Sampling, the ring, the histogram and every diagnostic have
    // already run above; only the declaration is refused. Everything that
    // would have been a candidate is counted into the same suppressed_ total
    // the refractory uses, and separately into a per-phase total, so this can
    // never hide a magnet in silence.
 ...
 private:
  ...
  void noteRest(uint32_t nowMs) {
    // X20. FROZEN FOR THE WHOLE STOP. This single line is the Arches fix: a
    // level that is held because the locomotive is parked in a fringe field is
    // not rest, however long it holds, and re-basing the zero onto it is what
    // turned that field's own wander into three magnets.
    if (restFrozen_) { quietRun_ = 0; return; }
    const uint16_t want = cfg_.localWindowMs ? cfg_.localWindowMs : 1;
  ...
  // ------------------------------------------------------------------------
  // THE STATION DWELL. Four private helpers and no new constant.
  // ------------------------------------------------------------------------

  // Arm normal detection RIGHT NOW. The dwell broke the refractory's timing
  // continuity -- a 500 ms detect-to-detect guard measured across a 30 s stop
  // describes nothing -- and the next real magnet can be under the sensor
  // within 500 ms of departure. Carrying the timer across would be blindness
  ...
  // The ramped PWM has just reached 0 inside a station stop.
  void beginDwell(uint32_t nowMs, int16_t raw) {
    dwell_           = true;
    dwellBeganMs_    = nowMs;
    dwellSuppressed_ = 0;
    // An in-flight window is truncated at this sample by the acquiring branch.
    truncateNow_     = acquiring_;
    // The anchor is the rest measured WHILE MOVING, and it does not move again
    // until the locomotive is demonstrably out of whatever it stopped in.
    dwellRest_       = restValid_ ? restRef_ : baseline_;
    dwellRestValid_  = restValid_;
    restFrozen_      = true;
    const int32_t d  = (int32_t)raw - dwellRest_;
    const int32_t m  = d < 0 ? -d : d;
    // Classify the STATE, never a magnet. With no measured rest to compare
    // against there is no basis for claiming a field, and the safe answer is
    // REST -- which arms on departure. Claiming IN_OLD_FIELD on no evidence
    // would hold detection off with nothing to release it.
    dwellDisplaced_     = dwellRestValid_ && m >= cfg_.departCounts;
  ...
  // The station machine has left the dwell: either it departed, or AUTO was
  // withdrawn under it. Both mean the same thing here.
  void endDwell(uint32_t nowMs) {
    dwell_ = false;
    if (dwellDisplaced_) {
      // B. Still inside the field already encountered while moving. The
      // changing values on the way out of it ARE that field. Rest stays
      // frozen, so the departure arithmetic keeps measuring from the true
      // line, and nothing is declared until the signal is back at it.
      oldFieldHold_    = true;
      oldFieldSinceMs_ = nowMs;
      holdSuppressed_  = 0;
      clearRun_        = 0;
    } else {
      // A. Stopped at rest. Nothing to leave; arm on the spot.
      restFrozen_ = false;
      arm();
    }
  ...
  // Resting territory reached: within one departure threshold of the frozen
  // rest, for the same persistence any candidate must satisfy. Both numbers
  // are already in use; neither is new.
  void checkOldFieldClear(uint32_t nowMs, int16_t raw) {
    ...
    oldFieldHold_ = false;
    clearRun_     = 0;
    restFrozen_   = false;      // noteRest resumes; it has its own 800 ms
    arm();
 ...
 private:
  ...
  // X20 dwell state.
  bool     dwell_ = false, dwellDisplaced_ = false, dwellRestValid_ = false;
  bool     oldFieldHold_ = false, restFrozen_ = false, truncateNow_ = false;
