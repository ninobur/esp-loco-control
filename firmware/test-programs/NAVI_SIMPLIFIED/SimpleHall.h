...
struct HallDecision {
  ...
  uint8_t polarity = 0;
  uint8_t pwm = 0;
  bool zeroDuringCandidate = false;
};
...
class SimpleHall {
  ...
  HallDecision sample(uint32_t now, int16_t raw, uint8_t pwm) {
    ...
    if (state_ != State::Open) {
      if (above) {
        ...
        if (overCount_ && sign != candidateSign_) overCount_ = 0;
        if (++overCount_ == 1) { firstOverMs_ = now; zeroDuringCandidate_ = pwm == 0; }
        candidateSign_ = sign;
        if (pwm == 0) zeroDuringCandidate_ = true;
        if (overCount_ >= 2) {
          ...
          state_ = State::Open;
          openedMs_ = now; // the second qualifying sample declares detection
          ...
          out.zeroDuringCandidate = zeroDuringCandidate_;
          return out;
        }
      } else { overCount_ = 0; zeroDuringCandidate_ = false; }
    }
 ...
 private:
  ...
  bool zeroDuringCandidate_ = false;
