...
//
// Runs the ACTUAL X19 ExcursionDetector, the same header the sketch compiles,
...
int main(int argc, char** argv) {
  ...
  // census magnets. X19 has no width screen at all, so it must find both -- and
  ...
  for (const auto& r : recs) {
    ...
    // refused. X19 has no floor; this is the risk, counted.
    for (auto& c : cands)
  ...
  printf("\n--- X19 implementation vs the 2026-09-15 replay ---\n");
